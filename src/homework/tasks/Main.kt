...
/*
*
* 1. Неизменяемость полей задачи
* 2. Инкапсуляция списков задач и архива
* 3. Архивирование изменений
* 4. Архивирование изменений
*
* */
class Task(
    val id: Int,
...
class Employee(val id: Int, val name: String) {
    ...
    fun updateTaskStatus(taskId: Int, newStatus: String) {
        val task = _tasks.find { it.id == taskId }
        if (task != null) {
            val newTask = task.copy(status = newStatus)  // Прямое изменение, требующее изменения на использование copy()
    ...
    fun changeTaskAssignee(taskId: Int, newAssignee: String) {
        val task = _tasks.find { it.id == taskId }
        if (task != null) {
            val newTask = task.copy(assignedTo = newAssignee)  // Прямое изменение, требующее изменения на использование copy()
            updateTaskById(taskId, newTask)
            _archivedTasks.add(task)
    ...
    fun updateTaskPriority(taskId: Int, newPriority: String) {
        val task = _tasks.find { it.id == taskId }
        if (task != null) {
            val newTask = task.copy(priority = newPriority)  // Прямое изменение, требующее изменения на использование copy()
            updateTaskById(taskId, newTask)
            _archivedTasks.add(task)
    ...
    fun modifyTaskDetails(taskId: Int, newTitle: String, newDescription: String) {
        val task = _tasks.find { it.id == taskId }
        if (task != null) {
            val newTask = task.copy(title = newTitle, description = newDescription)  // Прямое изменение, требующее изменения на использование copy()
            updateTaskById(taskId, newTask)
            _archivedTasks.add(task)
    ...
    private fun updateTaskById(taskId: Int, newTask: Task) {
